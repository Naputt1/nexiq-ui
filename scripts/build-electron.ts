...
try {
  ...
  if (!fs.existsSync(join(REPO_ROOT, 'dist'))) {
    throw new Error(`Build failed: 'dist' directory not found at ${join(REPO_ROOT, 'dist')}`);
  }
  if (!fs.existsSync(join(REPO_ROOT, 'dist-electron'))) {
    throw new Error(`Build failed: 'dist-electron' directory not found at ${join(REPO_ROOT, 'dist-electron')}`);
  }
  ...
  run(`pnpm exec electron-builder build ${args}`, join(REPO_ROOT, 'out-build'));
} catch (error) {
