...
export const ViewSwitcher: React.FC<ViewSwitcherProps> = memo(
  ({ className, isCollapsed, compact = false }) => {
    ...
    return (
      ...
      >
        ...
        <Select
          value={view}
          onValueChange={(val) => setView(val as GraphViewType)}
        >
          <SelectTrigger
            className={cn("h-8 text-xs", compact ? "w-36" : "w-full")}
          >
